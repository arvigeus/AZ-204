...
#### [Access restriction policies](https://learn.microsoft.com/en-us/azure/api-management/api-management-policies)
...
#### Client Certificates in Consumption Tier

In the _Consumption_ tier of API Management, suited for serverless APIs, the use of client certificates must be manually enabled on the **Custom domains** page.

#### Check the thumbprint against certificates uploaded to API Management

Use `<when condition="$(...)">` policy:

```cs
// Checks if the request doesn't contain a certificate or if the thumbprint of the certificate doesn't match the desired one
context.Request.Certificate == null || context.Request.Certificate.Thumbprint != "desired-thumbprint"

// Checks if the request doesn't contain a certificate, if the certificate cannot be verified, or if none of the deployment certificates' thumbprints match the request's certificate thumbprint
